...
export default function InvoicesPage() {
  ...
  const handleUpdateDataSent = async (update: DataSentUpdate) => {
    try {
      ...
      if (result.success) {
        console.log('Invoices - API response data:', result.data);
        console.log('Invoices - ordered_by_provider_at from API:', result.data.ordered_by_provider_at);
  ...
  const handleViewInvoice = (invoiceId: string) => {
    ...
    if (invoice) {
      ...
      window.location.href = `/invoices/${invoice.mx_invoice_id}`;
    }
  };
