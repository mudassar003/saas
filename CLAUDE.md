...
```bash
# .env.local
NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_test_dG9sZXJhbnQtYWlyZWRhbGUtNTkuY2xlcmsuYWNjb3VudHMuZGV2JA
CLERK_SECRET_KEY=sk_test_h9Lay44ZtTOEB0cQQ5CUjpBTHN6ZGGZo79kc07e6Gp
```
...
1. code should be as minimal as possible with as minimal files and old files modification quality but less code than a lot of new files or code and we are following enterprise level
2. code should follow strict typescript writing rules with interfaces and types and next js 15 rules
3. code should not have any vercel issues
4. api should follow strict latest security protocols
5. you will write code as a senior software develeoper
7. complete webapp should be protected this is only for logged in users